...
class FeatureAlignModule(nn.Module):
    def __init__(self, in_channels, kernel_size):
        ...
        self.conv0 = nn.Sequential(
            nn.Conv2d(2*in_channels, 2*in_channels, kernel_size=kernel_size, stride=1, padding=kernel_size//2),
            nn.BatchNorm2d(2*in_channels),
            nn.ReLU())
        self.conv1 = nn.Sequential(
            nn.Conv2d(2*in_channels, 2*in_channels, kernel_size=1, stride=1, padding=0),
            nn.Sigmoid())
        self.conv2 = nn.Conv2d(in_channels, in_channels, kernel_size=kernel_size, stride=1, padding=kernel_size//2)
...
class CrossingAttentionPAFPN(nn.Module):
    """
    YOLOv8 model. Darknet 53 is the default backbone of this model.
    """

    def __init__(self,in_channels=[256, 512, 1024],out_channels=256):  # model, input_channels, depth_multiple, width_multiple
        super().__init__()
        self.laterial_convs = nn.Sequential()
        for i in range(len(in_channels)):
            self.laterial_convs.append(DeepCrossingAttention(in_channels[i], out_channels))
        self.align_convs = nn.Sequential()
        for i in range(len(in_channels)-1):
            self.align_convs.append(FeatureAlignModule(out_channels, kernel_size=5))
        self.convert_align_convs = nn.Sequential()
        for i in range(len(in_channels)-1):
            self.convert_align_convs.append(FeatureAlignModule(out_channels, kernel_size=5))
    ...
    def forward(self, x):
        ...
        for i in range(len(x)):
            features.append(self.laterial_convs[i](x[i]))
            print(features[i].shape)
        for i in range(-1, -len(features), -1):
            prev_shape = features[i-1].shape[2:]
            features[i-1] = self.align_convs[i](F.interpolate(features[i],size=prev_shape,mode="nearest"), features[i-1])
...
if __name__ == "__main__":
    x = [torch.randn(1, 256, 128, 128), torch.randn(1, 512, 64, 64), torch.randn(1, 1024, 32, 32)]
    model = CrossingAttentionPAFPN(in_channels=[256, 512, 1024],out_channels=256)
    y = model(x)
    print(y[0].shape)
    print(y[1].shape)
    print(y[2].shape)